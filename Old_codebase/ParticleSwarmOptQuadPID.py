...
def render( mode='human'):
    global  renderCount,ax, databaseName
    ax.cla()
    renderCount+=1
...
def saveData(pos , fitness):
    datapoint = {'ParticleParameters': { 'P': pos[0], 'D':pos[1]} , 'total_loss': fitness}
    database.append(datapoint)
    print(database)

    with open('PSO_output.json', 'w') as outfile:
